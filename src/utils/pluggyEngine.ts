import {
  Category, Subcategory, Tag, Account, Transaction, AutomationRule, TransactionType,
  PluggyPendingTx,
} from '../types';
import { applyRulesToTx, RuleContext } from './ruleEngine';
import { RawPluggyTx, rawToPendingFields, findReconciliationMatch } from './pluggy';
...
export function buildApprovedTransaction(
  ...
  ctx: PluggyEngineContext,
  overrides?: Partial<Pick<Transaction, 'categoryId' | 'category' | 'subcategoryId' | 'subcategory' | 'tagIds' | 'amount' | 'date' | 'notes' | 'accountId' | 'memberId'>>
): Transaction {
